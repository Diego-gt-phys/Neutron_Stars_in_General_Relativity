...
PCS = {"soft": (1e-10, 5e-3), "middle": (2.747e-6, 5.713e-4), "stiff": (2.144e-6, 2.802e-4)} # Central pressure intervals for the MR curves 
...
def Save_MR (s_type, eos_c, dm_m, p2_c, p2_v, p_data, rho_data, p_data_dm, rho_data_dm):
    ...
    if s_type != 3:
        R, M = MR_curve_1f(pc_range, s_type, (1e-6, 100), 1e-3, 50, p_data, rho_data, p_data_dm, rho_data_dm)
        data["R"] = R
        data["M"] = M
        df = pd.DataFrame(data)
        if s_type == 1:
...
def find_alpha_max(p1_c, p1_v, p_data, rho_data, p_data_dm, rho_data_dm):
    ...
    result = opt.minimize_scalar(neg_lambda, bounds=(0, 11), method='bounded')
...
###############################################################################
    ...
    mode, s_type, d_type, eos_c, dm_m, p1_c, p1_v, p2_c, p2_v = get_inputs(1, 2, 0, 'soft', 0.93956542, 'M', 0.5, 'None', None)
...
###############################################################################
    ...
    if mode == 1:
        ...
        if d_type == 0:
            ...
            # Plot Mass
            ax2 = ax1.twinx()
            m_line = ax2.plot(r, m, label=r'$m(r)$', color = 'k', linewidth=1.5, linestyle='--')
            if s_type == 3:
