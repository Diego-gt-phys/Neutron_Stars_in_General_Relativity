# -*- coding: utf-8 -*-
"""
Created on Sat Dec  6 19:12:26 2025

Using data calculated through DANTE.py it plots the preassures and mass profiles of a Neutron Star.
This plot contain different hydrostatic solutions of different mass for the EoS middle.
...
eos_c = 'middle'
p1_c = 'M'
p1_v_list = [0.5, 1.0, 1.5, 2.0]

for p1_v in p1_v_list:
    df = pd.read_csv(rf"data\{s_type}_{d_type}_{eos_c}_{p1_c}_{p1_v}.csv")
    data[f'{p1_v}'] = df
...
# The plot thickenss
for p1_v in p1_v_list:
    ax1.plot(data[f"{p1_v}"]['r'], data[f"{p1_v}"]['p_A'], color = colors[c], linewidth=1.5, linestyle='-')
    ax2.plot(data[f"{p1_v}"]['r'], data[f"{p1_v}"]['m'], color = colors[c], linewidth=1.5, linestyle='--')
    c+=1

if True == True:
    ax1.set_xlim(0, 12.47)
...
if True == True:
    ax1.set_xticks(np.arange(0, 12.47, 1))
    #ax1.set_xticks(np.arange(0, 9.6, 0.2), minor=True)
    ax1.set_yticks(np.arange(0, 1.21e-4, 0.2e-4))
    #ax1.set_yticks(np.arange(0, 8.1e-5, 0.2e-5), minor=True)
    ax2.set_yticks(np.arange(0, 2.00, 0.25))
...
handles_list = [
    mlines.Line2D([], [], color=colors[0], linestyle='-', label=r"$M=0.5$"),
    mlines.Line2D([], [], color=colors[1], linestyle='-', label=r"$M=1.0$"),
    mlines.Line2D([], [], color='k', linestyle='-', label=r"$p(r)$"),
    mlines.Line2D([], [], color=colors[2], linestyle='-', label=r"$M=1.5$"),
    mlines.Line2D([], [], color=colors[3], linestyle='-', label=r"$M=2.0$"),
    mlines.Line2D([], [], color='k', linestyle='--', label=r"$m(r)$")]

plt.legend(handles=handles_list, loc = "upper center", bbox_to_anchor=(0.5, 0.99), fontsize=15, frameon=True, fancybox=False, ncol = 2, edgecolor="black", framealpha=1, labelspacing=0.2, handletextpad=0.3, handlelength=1.4, columnspacing=1)
