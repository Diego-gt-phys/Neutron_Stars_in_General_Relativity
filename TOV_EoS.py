# -*- coding: utf-8 -*-
"""
Created on Sat Dec  6 19:59:41 2025

Using data calculated through DANTE.py it plots the preassures and mass profiles of a Neutron Star.
This plot contain different hydrostatic solutions of different EoS and same mass (M=1.5).
...
# The plot thickenss
for eos_c in eos_c_list:
    ax1.plot(data[f"{eos_c}"]['r'], data[f"{eos_c}"]['p_A'], color = colors[c], linewidth=1.5, linestyle='-')
    ax2.plot(data[f"{eos_c}"]['r'], data[f"{eos_c}"]['m'], color = colors[c], linewidth=1.5, linestyle='--')
    c+=1
...
if True == True:
    ax1.set_xticks(np.arange(0, 14.01, 1))
    #ax1.set_xticks(np.arange(0, 9.6, 0.2), minor=True)
    ax1.set_yticks(np.geomspace(1e-14, 1e-4, 11))
...
plt.legend(handles=handles_list, loc = "center left", bbox_to_anchor=(0.01, 0.5), fontsize=15, frameon=True, fancybox=False, ncol = 2, edgecolor="black", framealpha=1, labelspacing=0.2, handletextpad=0.3, handlelength=1.4, columnspacing=1)

# Save plot as PDF
#plt.title(rf'DM Stars, Relativity check. $m_{{\chi}} = {dm_m}$', loc='left', fontsize=15, fontweight='bold')
plt.tight_layout()
plt.savefig(f"figures\TOV_EoS.pdf", format="pdf", bbox_inches="tight")
